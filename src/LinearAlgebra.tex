...
\usepackage{xcolor}
\usepackage{multicol}
\usepackage{systeme} % typeset systems of equations
	\sysdelim..
...
\newcommand{\xhat}{{\sbasis{1}}}
\newcommand{\yhat}{{\sbasis{2}}}
\newcommand{\zhat}{{\sbasis{3}}}
...
\makeatletter
	...
	\declaretheorem[numberwithin=section,
			style=orangenumbox,
			preheadhook={\extdisable\begin{leftborderbox}[colframe=myorange, colback=myorange!20]},
			postfoothook={\end{leftborderbox}\extenable}]{theorem}

	\theoremstyle{ocrenumbox}
	\declaretheorem[sibling=theorem,
			style=ocrenumbox,
			preheadhook={\extdisable\begin{leftborderbox}[colframe=mypink, colback=mypink!10]},
			postfoothook={\end{leftborderbox}\extenable}]
			{definition}
	...
	\declaretheorem[sibling=example,
			style=ocrenumbox,
			preheadhook={\extdisable\begin{leftborderbox}[colframe=mypink, colback=white]},
			postfoothook={\end{leftborderbox}\extenable}]
			{exercise}
	...
	% background for the end-of-chapter exercises
	\newtcolorbox{exsbox}[1][]{
		blanker,
		frame hidden,
		enhanced,
		breakable,
	...
	\newtcolorbox{probbox}[1][]{
		...
		top=5pt,
		bottom=5pt,
		borderline west={1pt}{0pt}{tcbcol@frame},
		colback=black!3,
	}



	% MAKE AN ENVIRONEMENT FOR PROBLEM LISTS
	% the parts environment should act quite similarly to the
	% list environment, so we'll use the latex list environment
	% to create it
	\newcounter{probcounter}[chapter] % reset this counter every chapter
	\newcounter{partsitem}
	\newcounter{partsitemlast}
	\newcommand{\formatpartsitem}{%
		{\color{gray}{\sffamily\small\thequestion.\thepartsitem}}
	}
	\DeclareDocumentEnvironment{question}{o}{
		\nopagebreak
		\stepcounter{probcounter}
		\extdisable% disable tikz externalization for tcolorbox's sake
		\begin{probbox}[\thechapter.\theprobcounter]
	...
	\DeclareDocumentEnvironment{parts}{o}{
		\nopagebreak
		\setlist[enumerate, 2]{label=({\itshape\roman*})}
		\begin{enumerate}[
			label={\color{mypink}(\alph*)},
			leftmargin=17pt,
			labelsep=2.5pt,
			topsep=0pt,
			partopsep=0pt,
...
\newenvironment{exercises}{%
	\newpage
	\begin{multicols*}{2}[{\extdisable\begin{tcolorbox}[enhanced, frame hidden, colback=blue!10, sharp corners]{\subsection*{Exercises for \thesection}}\vspace{-0pt}\end{tcolorbox}\extenable}]\small %
	}{%
	\end{multicols*}%
	\clearpage
}
...
\chapter{Vector Spaces}
	\input{chapters/chapterII1.tex}
	\clearpage
\chapter{Linear Transformations}
	\input{chapters/chapterII2.tex}
	\clearpage
\chapter{Matrices}
	\input{chapters/chapterII3.tex}
	\clearpage
\chapter{Inner Product Spaces}
	\input{chapters/chapterII4.tex}
	\clearpage
