...
\begin{exercises}
	\begin{enumerate}
		\item   In each case determine if the given pair of vectors is perpendicular.
			\begin{enumerate}
		...
		\item   An inclined plane makes an angle of $30$ degrees with the horizontal.  Use
			...
			acceleration vector $-g\yhat$ along a unit vector pointing down the inclined
			plane.
...
\begin{openstaxexercises}
	\begin{enumerate}
		\item	For the following exercises, the vectors $\vec{u}$ and $\vec{v}$ are given. Find
			the projection $w = \Proj_{\vec{u}}\vec{v}$ of vector $\vec{v}$ onto vector
		...
		\item	Consider the vectors $\vec{u} = 4\xhat − 3\yhat$ and $\vec{v} = 3\xhat + 2\yhat$
			\begin{enumerate}
				\item	Find the component form of vector $\vec{w} =
					\Proj_{\vec{u}}\vec{v}$ that represents the projection of
					$\vec{v}$ onto $\vec{u}$.
