...
\begin{exercises}
	\begin{problist}
		\prob   Find the vector of the form $a\xhat + b\yhat + c\zhat$ that is represented by an 
...
\begin{exercises}
	\begin{problist}
		...
		\prob   An inclined plane makes an angle of $30$ degrees with the horizontal.  Use
			...
			plane.
		\prob   Show that if the velocity vector $\vec{v}$ is perpendicular to the acceleration
		...
		\prob[\openstax]	For the following exercises, the vectors $\vec{u}$ and $\vec{v}$ are given. Find
			...
			\end{enumerate}
		\prob[\openstax]	Consider the vectors $\vec{u} = 4\xhat − 3\yhat$ and $\vec{v} = 3\xhat + 2\yhat$
			\begin{enumerate}
