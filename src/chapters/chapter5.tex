...
\begin{question}
	To get our bearings, let's revisit integrating with respect to $\mathrm{d}x$ and $\mathrm{d}y$
...
\begin{question}
	\begin{parts}
		\item Draw the lines $x=x_0$, $x=x_0+\Delta x$, $y=y_0$ and $y=y_0+\Delta y$ where
			$\Delta x$ and $\Delta y$ are assumed to be small.  Write down a formula for the area enclosed by
...
\begin{question}
	...
	\begin{parts}
		\item Write down the bounds of $D$ in rectangular coordinates and set up (but don't evaluate)
			an integral for $B$.
		\item Write down the bounds of $D$ in polar coordinates and set up (but don't evaluate) 
			an integral for $B$.
...
\begin{question}
	...
	\begin{parts}
		...
		\item A discus is constructed from gluing two cones' bases together.
			The radius of the base of each cone is $4$ and the height of each
			cone is $1$.  Describe the surface of the discus using cylindrical coordinates
			(you may need to describe the top and bottom separately)
		\item The discus has a density given by $\rho(x,y,z) = x^2+y^2+2|z|$.  Find the
			mass of the discus. (You may exploit symmetry.)
	\end{parts}
\end{question}
...
the idea that ``$\d x$'' represents an infinitesimally small change in the variable $x$ gives
...
As always, the purpose of notation is to facilitate our thinking, not to be a formal grammar\footnote{
	...
	most programming languages could be thought of as very stringent 
...
\]
It's now not so far fetched to believe
\[
